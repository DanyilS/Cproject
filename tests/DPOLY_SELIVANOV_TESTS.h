//
// Created by Селиванов Даниил, Салтыков Илья.
//
...
#define N 10  //constant value for all tests
...
int dpoly_sinput_test_1(void){
    ...
    if(poly.coefficients[0]!=3.||poly.coefficients[1]!=-0.5||poly.coefficients[2]!=0.||poly.coefficients[3]!=140.)
    {
        dpoly_destroy(&poly);
        return 1;
    }
...
int dpoly_sinput_test_2(void){
    ...
    if(poly.coefficients[0]!=0||poly.coefficients[1]!=1||poly.coefficients[2]!=5){
        dpoly_destroy(&poly);
        return 1;
    }
    dpoly_destroy(&poly);
